...
一个基于 C++17 与 [raylib](https://www.raylib.com/) 开发的 2D **迷宫解谜游戏**，包含推箱子、活塞、水流与岩浆扩散、敌人追踪等要素。支持 Windows 平台。
...
游戏共有 10 关，在每一关中，从玩家起点出发，躲避岩浆和敌人，推动箱子、触发活塞，最终抵达终点宝箱即可通关。
...
- **水流与岩浆**：
  ...
  - 岩浆会缓慢扩散，玩家接触岩浆会死亡，箱子接触岩浆时会消失，水流和岩浆相遇时会生成石头。
- **活塞与按钮**：
  - 按钮被玩家，箱子或敌人按下时会启动对应的活塞。
  - 需要多个同色按钮同时按下才会启动活塞。
  - 活塞可以推动箱子，改变地图的通路。
- **敌人**：
  - 敌人使用 BFS 在可行走区域上寻找追踪玩家的最短路径，若无法找到到达玩家的路径则停止移动。
  - 敌人移动速度为玩家的一半，接触到玩家时会导致玩家失败。
...
- 游戏后几关较难，但保证每关都有解。
- 游戏内置了可选的开挂操作，连按五次空格后激活。激活之后可以通过按空格发射子弹击晕敌人，在卡关时可以以此降低游戏难度。
- 本游戏有存档功能，放置于 `assets/saves/saves.txt`。若卡关，可以自行修改存档的数值来实现跳关，也可以用其清空存档。
